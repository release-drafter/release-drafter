...
| `label-versioning`    | Optional | Version a release on merge to the default branch. By default, the version will be incremented by a patch version, unless the labels 'MINOR' or 'MAJOR' are present on any of the PRs that have gone into the release, in which case the version will be bumped accordingly. Includes a sub-option `auto-release` to automatically release the project at that version. |
