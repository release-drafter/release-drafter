...
const resolveVersionKeyInc = (mergedPullRequests, config) => {
  const priorityMap = {
    patch: 1,
    minor: 2,
    major: 3
  ...
  const keys = mergedPullRequests.flatMap(pr =>
    pr.labels.nodes.map(
      node => labelToKeyMap[node.name] || config['version-resolver'].default
    )
  )
...
module.exports.generateReleaseInfo = ({
  ...
  const versionInfo = getVersionInfo(
    ...
    version || tag || name,
    resolveVersionKeyInc(mergedPullRequests, config)
  )
