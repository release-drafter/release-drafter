...
const getCategoriesConfig = ({ config }) => {
  ...
  const orderedCategories = [
    ...
    categories.push(...category.labels)
...
const categorizePullRequests = (pullRequests, config) => {
  const excludeLabels = config['exclude-labels']
  const [categories, orderedCategories] = getCategoriesConfig({ config })
  ...
  const uncategorizedPullRequests = pullRequests.filter(filterPullRequests)
