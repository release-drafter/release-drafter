...
const generateReleaseInfo = ({
  ...
  if (discussion_category_name === undefined) {
    discussion_category_name = config['discussion-category-name'] || ''
  }
...
const createRelease = ({ context, releaseInfo }) => {
  return context.octokit.repos.createRelease(
    context.repo({
      ...
      discussion_category_name: releaseInfo.discussion_category_name,
    })
  )
}
