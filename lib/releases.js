...
const contributorsSentence = ({ mergedCommits, pullRequests, config }) => {
...
const escapeTitle = (title, config) =>
...
const commitToString = (mergedCommits, config, owner, repo) => {
  if (!mergedCommits || mergedCommits.length === 0) {
    return ''
  }
  ...
  return mergedCommits
    .map((commit) => {
      ...
      const commitAuthor = commit.author.user
        ? commit.author.user.login
        : commit.author.name
      const shortHash = commit.oid.slice(0, 7)
      const commitUrl = `https://github.com/${owner}/${repo}/commit/${commit.oid}`

      return template(config['change-template'], {
        $TITLE: escapeTitle(
          parsed.subject || commit.message.split('\n')[0],
          config
        ),
        $AUTHOR: commitAuthor,
        $NUMBER: `[${shortHash}](${commitUrl})`,
        $BODY: parsed.body || '',
        $URL: commitUrl,
        $BASE_REF_NAME: '',
        $HEAD_REF_NAME: '',
...
const generateChangeLog = (
  ...
  const hasCommits =
    config['include-commits'] && mergedCommits && mergedCommits.length > 0

  if (mergedPullRequests.length === 0 && !hasCommits) {
  ...
  // Categorize commits if enabled
  let categorizedCommitsData = {
    categorizedCommits: [],
    uncategorizedCommits: [],
  }
  if (hasCommits) {
    const commitsWithoutPRs = filterCommitsWithoutPullRequests(mergedCommits)
    categorizedCommitsData = categorizeCommits(
      commitsWithoutPRs,
      config.categories
    )
  }

  const { categorizedCommits, uncategorizedCommits } = categorizedCommitsData
  ...
  const pullRequestToString = (pullRequests) =>
    pullRequests
      .map((pullRequest) => {
        ...
        return template(config['change-template'], {
          $TITLE: escapeTitle(pullRequest.title, config),
  ...
  // Handle uncategorized pull requests
  ...
  if (uncategorizedCommits.length > 0) {
    ...
    changeLog.push(commitToString(uncategorizedCommits, config, owner, repo))
  }
  ...
  for (const [index, category] of categorizedPullRequests.entries()) {
    const commitCategory = hasCommits ? categorizedCommits[index] : null

    const hasPRs = category.pullRequests.length > 0
    const hasCommitsInCategory =
      commitCategory && commitCategory.commits.length > 0

    if (!hasPRs && !hasCommitsInCategory) {
      continue
    }
    ...
    // Combine PRs and commits for this category
    const allItems = []

    if (hasPRs) {
      allItems.push(pullRequestToString(category.pullRequests))
    }

    if (hasCommitsInCategory) {
      allItems.push(commitToString(commitCategory.commits, config, owner, repo))
    }

    const combinedString = allItems.join('\n')
    const totalCount =
      category.pullRequests.length +
      (hasCommitsInCategory ? commitCategory.commits.length : 0)

    // Determine the collapse status.
    const shouldCollapse =
      category['collapse-after'] !== 0 &&
      totalCount > category['collapse-after']
    ...
    if (shouldCollapse) {
      changeLog.push(
        '<details>',
        '\n',
        `<summary>${totalCount} changes</summary>`,
        '\n\n',
        combinedString,
        '\n',
        '</details>'
...
const resolveVersionKeyIncrement = (
  ...
  // Check commits for version increment if enabled
  if (config['include-commits'] && mergedCommits && mergedCommits.length > 0) {
    const commitsWithoutPRs = filterCommitsWithoutPullRequests(mergedCommits)

    // Check if commit-types are configured
    const hasCommitTypes =
      (config['version-resolver'].major &&
        config['version-resolver'].major['commit-types']) ||
      (config['version-resolver'].minor &&
        config['version-resolver'].minor['commit-types']) ||
      (config['version-resolver'].patch &&
        config['version-resolver'].patch['commit-types'])

    if (hasCommitTypes && commitsWithoutPRs.length > 0) {
      const commitIncrement = resolveVersionFromCommits(
        commitsWithoutPRs,
        config
      )

      core.debug('commitIncrement: ' + commitIncrement)

      // Take the maximum increment between PRs and commits
      if (priorityMap[commitIncrement] > priorityMap[versionKeyIncrement]) {
        versionKeyIncrement = commitIncrement
      }
    }
  }
