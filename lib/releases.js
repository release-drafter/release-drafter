...
const templateNextVersion = (input, nextVersions, versionTemplate) => {
  ...
  const versions = increments.reduce((acc, inc) => {
    acc[inc] = template(versionTemplate, nextVersions[inc]) || ''
    return acc
  }, {})
  return template(input, versions)
}
...
module.exports.generateReleaseInfo = ({
  ...
  body = template(body, {
    $PREVIOUS_TAG: _.get(lastRelease, 'tag_name') || '',
