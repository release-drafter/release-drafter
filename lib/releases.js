...
const templateNextVersion = (input, nextVersions, versionTemplate) => {
  ...
  return template(input, {
    $NEXT_MAJOR_VERSION: nextMajor,
    $NEXT_MINOR_VERSION: nextMinor,
    $NEXT_PATCH_VERSION: nextPatch
  })
}
...
module.exports.generateReleaseInfo = ({
  ...
  body = body.template({ $PREVIOUS_TAG: get(lastRelease, 'tag_name') || '' })

  if (mergedPullRequests.length === 0) {
    body = body.template({ $CHANGES: config['no-changes-template'] })
  } else {
    body = body.template({
