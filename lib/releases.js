...
const getCategoriesConfig = ({ config }) => {
  ...
  const orderedCategories = [UNCATEGORIZED, ...config.categories.map(category => category.label)]
...
const categorizePullRequests = (pullRequests, categories) => {
  return pullRequests.reduce((acc, pullRequest) => {
    if (pullRequest.labels.length === 0 ||
        !pullRequest.labels.some(label => categories.includes(label))) {
...
module.exports.generateReleaseBody = ({ commits, config, lastRelease, mergedPullRequests }) => {
  ...
  if (mergedPullRequests.length === 0) {
    ...
    const categorizedPullRequests = categorizePullRequests(mergedPullRequests, orderedCategories)

    body = body.replace('$CHANGES', orderedCategories.reduce((acc, category, index) => {
      if (!categorizedPullRequests[category] ||
          categorizedPullRequests[category].length === 0) return acc
