...
const { getVersionInfo, incrementVersionBasedOnLabels } = require('./versions')
...
module.exports.generateReleaseInfo = ({
  ...
  if (lastRelease) {
    ...
    if (versionInfo) {
      ...
      if (!labelVersioningConfig.enabled) {
        name = template(name, versionInfo)
        tag = template(tag, versionInfo)
      } else {
        const nextVersion = incrementVersionBasedOnLabels(
          lastRelease,
          mergedPullRequests,
          labelVersioningConfig
        )
        // NOTE -- we do not respect any name template or tag template here,
        // because I'm not sure it makes sense to in the current form.
        name = nextVersion
        tag = nextVersion
      }
    }
    ...
    // What should auto-release do in this case?
    return {
      name: '',
      tag: '',
      body
