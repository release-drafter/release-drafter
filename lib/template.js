...
export const template = (string, object, customReplacers) => {
...
export function validateReplacers({ context, replacers }) {
  return replacers
    .map((replacer) => {
      try {
...
export function validateAutolabeler({ context, autolabeler }) {
  return autolabeler
    .map((autolabel) => {
      try {
        return {
