import { C as union, D as error, E as debug, M as warning, N as __commonJSMin, O as getInput, P as __toESM, S as stringbool, T as context, _ as boolean, a as commitishToCommitExpression, b as object, c as FindRecentMergedPullRequestsDocument, f as getPullRequestsChangedFiles, g as array, h as _enum, i as sharedInputSchema, j as setOutput, k as info, l as executeGraphql, m as ZodDefault, n as stringToRegex, o as parseCommitishForRelease, p as composeConfigGet, r as escapeStringRegexp, s as FindCommitsInComparisonDocument, t as require_ignore, u as getGitHubContext, v as literal, w as getOctokit, x as string, y as number } from "./ignore.js";
...
var getConfig = async (configName, github) => {
	const { config, contexts } = github ? await composeConfigGet(configName, {
		repo: github.repo,
		ref: github.ref ?? ""
	}, github.octokit) : await composeConfigGet(configName, context);
...
var mergeInputAndConfig = (params) => {
	...
	const { commitish, latest, prerelease } = getParsedDefaults(config, params.ref ?? context.ref);
...
var findPullRequests = async (params) => {
	...
	info(`Finding commits between ${previousCommitish} and ${params.config.commitish}...`);
	...
	].some((template) => template?.includes("$NEW_CONTRIBUTORS")) ? await findNewContributorLogins(pullRequests, params.github ?? getGitHubContext()) : /* @__PURE__ */ new Set();
	info(`Found ${pullRequests.length} merged pull requests targeting ${repo.owner}/${repo.repo}${pullRequests.length > 0 ? `: ${pullRequests.map((pr) => `#${pr.number}`).join(", ")}` : "."}`);
	return {
		commits,
		newContributorLogins,
...
//#endregion
export { setActionOutput as a, getActionInput as c, buildReleasePayload as i, actionInputSchema as l, findPullRequests as n, mergeInputAndConfig as o, findPreviousReleases as r, getConfig as s, upsertRelease as t };
